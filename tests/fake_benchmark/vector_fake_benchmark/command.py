...
Run as a script, never imported by the plugin. It imports `vector_fake_simulator` first, the way a
...
`vector_policy.client.RemotePolicy`: hello, the demonstration without `meta`, reset, then one `act`
...
import vector_fake_simulator  # noqa: E402

from vector_fake_benchmark import RESULT_RESERVE_S  # noqa: E402
...
def materialize(args: argparse.Namespace) -> int:
    ...
    meta = {"task": args.task, "scene_seed": seed, "sim": vector_fake_simulator.NAME}
    arrays = {
...
def run(args: argparse.Namespace) -> int:
    ...
    if behaviour == "garbage":
        ...
        return 0
    prompt_sha256 = vector_fake_simulator.digest(Path(args.prompt))
    if behaviour == "policy":
...
def drive_policy(args: argparse.Namespace) -> dict:
    ...
    from vector_policy.client import PolicyUnavailable, RemotePolicy
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="vector-fake-benchmark")
