...
`vector_fake_simulator` - a stand-in for SAPIEN - so a test can check that loading and calling the
plugin never loads the "simulator".

It never imports `vector_orchestrator`, as the ABI requires of every benchmark.
