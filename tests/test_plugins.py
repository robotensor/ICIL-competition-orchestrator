...
from vector_orchestrator.benchmarks import plugins
from vector_orchestrator.benchmarks.plugins import BenchmarkRefused, discover, load
...
def variant(mutation: str) -> str:
    return VARIANT.format(
        path=str(FAKE_SITE / "vector_fake_benchmark" / "__init__.py"), mutation=mutation
    )
...
def test_the_fake_benchmark_is_found_by_entry_point_and_loads(fake_installed, fake_spec):
    ...
    assert (fake.distribution, fake.version, fake.entry_point) == (
        "vector-fake-benchmark",
        "0.1.0",
        "vector_fake_benchmark",
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "provided by distribution 'vector-fake-benchmark', but spec.json pins "
...
def test_a_pin_mismatch_refuses_the_plugin_before_importing_it(
    ...
    assert "vector_fake_benchmark" not in sys.modules, "a refused plugin was imported"
...
def test_the_wheel_sha256_is_checked_against_what_the_installer_recorded(
    ...
    install_distribution(
        ...
        entry_points={"fake": "vector_variant_ok"},
        modules={"vector_variant_ok": variant("    pass")},
        direct_url={
            "url": "file:///wheels/vector_variant_benchmark-0.1.0-py3-none-any.whl",
            "archive_info": {"hashes": {"sha256": wheel}},
        },
    )
    pin = {**FAKE_PIN, "distribution": "vector-variant-benchmark", "wheel_sha256": wheel}
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("    id = 'elsewhere'", "vector_variant_bad is advertised as 'fake' but calls itself"),
...
def test_a_plugin_that_is_not_a_usable_benchmark_is_refused_with_the_reason(
    ...
    install_distribution(
        "vector-variant-benchmark",
        entry_points={"fake": "vector_variant_bad"},
        modules={"vector_variant_bad": variant(mutation)},
    )
    spec = write_spec(
...
def test_a_plugin_that_imports_a_simulator_is_refused(install_distribution, spec_doc, write_spec):
    install_distribution(
        "vector-variant-benchmark",
        entry_points={"fake": "vector_variant_heavy"},
        modules={
            "vector_variant_heavy": "import mujoco\n" + variant("    pass"),
    ...
    assert "importing vector_variant_heavy imported mujoco; a plugin's pure half must not" in (
        fake.problems
    )
...
def test_two_distributions_claiming_one_name_are_refused(
    ...
    install_distribution(
        "vector-variant-benchmark",
        entry_points={"fake": "vector_variant_twin"},
        modules={"vector_variant_twin": variant("    pass")},
    )
...
def test_a_module_shadowing_the_pinned_distribution_is_refused_before_import(
    ...
    install_distribution(
        "vector-variant-benchmark",
        entry_points={"fake": "vector_variant_shadowed"},
        modules={"vector_variant_shadowed": variant("    pass")},
    )
    shadow = tmp_path / "checkout"
    shadow.mkdir()
    (shadow / "vector_variant_shadowed.py").write_text("raise SystemExit('shadow imported')\n")
    monkeypatch.syspath_prepend(str(shadow))
    spec = write_spec(
        fake_spec_doc(spec_doc, {**FAKE_PIN, "distribution": "vector-variant-benchmark"})
    )
    fake = discover(spec, load=True)["fake"]
    assert fake.benchmark is None and "vector_variant_shadowed" not in sys.modules
    (problem,) = fake.problems
    assert problem.startswith("vector_variant_shadowed resolves to ")
    assert str(shadow) in problem and "not a file of vector-variant-benchmark" in problem
...
def _install_with_record(site, name, module, code, wheel_url):
    ...
    (info / "entry_points.txt").write_text(f"[robotensor.benchmarks]\nfake = {module}\n")
...
def test_installed_files_must_still_be_the_pinned_wheel(
    ...
    name, module = "vector-variant-benchmark", "vector_variant_recorded"
    code = variant("    pass")
    wheel = tmp_path / "vector_variant_benchmark-0.1.0-py3-none-any.whl"
    site = tmp_path / "site"
...
def test_a_benchmark_in_the_group_we_replaced_is_still_found(
    install_distribution, spec_doc, write_spec
):
    """Third parties registered into `icil.benchmarks` before the competitions were named; a
    distribution that never re-released is found, and loaded, exactly as before."""
    install_distribution(
        "vector-variant-benchmark",
        entry_points={"fake": "vector_variant_legacy_group"},
        modules={"vector_variant_legacy_group": variant("    pass")},
        group="icil.benchmarks",
    )
    spec = write_spec(
        fake_spec_doc(spec_doc, {**FAKE_PIN, "distribution": "vector-variant-benchmark"})
    )
    fake = discover(spec, load=True)["fake"]
    assert fake.ok and fake.benchmark is not None
