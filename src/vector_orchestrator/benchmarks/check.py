"""`vector-orchestrator benchmarks check <id>`: can this host run that benchmark for the spec?
...
CHECK_AUTHKEY_ENV = "VECTOR_POLICY_AUTHKEY"
CHECK_ROOT = "/nonexistent/vector-orchestrator-check"
...
def _check_skill(
    ...
    kwargs = dict(
        seed_material=f"vector-orchestrator benchmarks check|{skill}",
        count=count,
        suite=suite,
        category=category,
    )
