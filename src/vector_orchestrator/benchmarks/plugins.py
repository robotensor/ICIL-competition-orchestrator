...
A benchmark advertises itself through the `robotensor.benchmarks` entry point group (and the
`icil.benchmarks` group it replaced, still read). Loading an entry
...
from .api import ENTRY_POINT_GROUP, LEGACY_ENTRY_POINT_GROUPS, validate_plugin
...
def _entry_points() -> list[Any]:
    """Every advertised benchmark, from the current group and the ones it replaced.

    A distribution that registers in both groups is one benchmark, not two: entry points are
    counted by what they name and what they load.
    """
    found: list[Any] = []
    ...
    for group in (ENTRY_POINT_GROUP, *LEGACY_ENTRY_POINT_GROUPS):
        for ep in metadata.entry_points(group=group):
            ...
            key = (ep.name, ep.value, str(getattr(dist, "name", "") or ""))
            if key not in seen:
                seen.add(key)
                found.append(ep)
    return found
