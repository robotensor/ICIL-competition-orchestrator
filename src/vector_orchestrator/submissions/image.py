...
from vector_policy.manifest import Manifest
...
SUBMISSION_IMAGE = "vector-submission"
...
INDEX_PROBE_IMAGE = "vector-index-probe"
...
def base_image(spec: Any, digest: str | None = None) -> BaseImage:
    ...
    if not chosen:
        raise SubmissionError(
            ...
            "build one with `vector-orchestrator submission build-base` and pass --base-image"
        )
...
def ensure_base(docker: Docker, base: BaseImage) -> str:
    ...
    if found is None:
        raise SubmissionError(
            ...
            "`vector-orchestrator submission build-base` or load it, then check its digest"
        )
...
def index_probe_dockerfile(base: BaseImage) -> str:
    ...
    download = (
        ...
        f"--dest /tmp/vector-index-probe {INDEX_PROBE_REQUIREMENT}"
    )
    return (
        f"# the orchestrator's index probe, from {base.digest}\n"
        f"FROM {base.tag}\n"
        f"RUN {download} && rm -rf /tmp/vector-index-probe\n"
    )
...
def probe_index(
    ...
    started = time.monotonic()
    with tempfile.TemporaryDirectory(prefix="vector-index-probe-") as context:
        try:
            docker.build(
                Path(context),
