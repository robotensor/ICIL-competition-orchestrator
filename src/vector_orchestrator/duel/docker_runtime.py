"""The duel's runtime seam, mapped onto the submission sandbox (`vector_orchestrator.submissions`).
...
- `prepare`: `check_repository` (the manifest as a plain file), `build_submission_image` from the
  ...
  removed.
- `serve`: one `PolicyContainer` per unit, named `vector-duel-<key>-<random>`, with a socket directory
