"""`vector-orchestrator` command line. argparse only, and every import is local to its command, so
...
def cmd_submission(args: argparse.Namespace) -> int:
    ...
    work = Path(args.work) if args.work else Path(tempfile.mkdtemp(prefix="vector-check-"))
    try:
...
def _orchestrator(args: argparse.Namespace, spec):
    ...
    if manifest is None:
        raise ValueError(f"{args.store} is not a store; run `vector-orchestrator store init` first")
...
def _add_duel_args(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument(
        "--policy-python",
        default=(
            os.environ.get("VECTOR_POLICY_PYTHON")
            or os.environ.get("ICIL_POLICY_PYTHON")  # what it was called before the rename
            or sys.executable
        ),
        help="the policy environment's interpreter (weights runtime; default "
        "$VECTOR_POLICY_PYTHON, else this one)",
    )
    p.add_argument(
        "--workers",
        type=int,
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="vector-orchestrator", description="Robotensor Vector orchestrator"
    )
    ...
    dm.add_argument(
        ...
        "command line (default: VECTOR_ADMIN_TOKEN)",
    )
    _add_duel_args(dm)
