...
`accepted_at` and `message` from an acceptance; and it stops waiting after `VECTOR_ADMIN_TIMEOUT_MS`,
...
DEFAULT_PORT = 8799
DEFAULT_TOKEN_ENV = "VECTOR_ADMIN_TOKEN"
...
#: 503: the dashboard waits `VECTOR_ADMIN_TIMEOUT_MS`, 6 s unless set, and has reported a timeout by
...
class AdminServer:
    ...
    def start(self) -> threading.Thread:
        thread = threading.Thread(target=self.httpd.serve_forever, name="vector-admin", daemon=True)
        self._served = True
        thread.start()
        self._thread = thread
        return thread
...
def _handler(server: AdminServer, timeout_s: float) -> type[BaseHTTPRequestHandler]:
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        server_version = "vector-orchestrator-admin"
        sys_version = ""
        timeout = timeout_s
        ...
        def _answer(self) -> tuple[int, dict[str, Any], bool]:
            ...
            if len(named) != 1 or not server.authorized(named[0]):
                raise Refused(
                    ...
                    headers={"WWW-Authenticate": 'Bearer realm="vector-orchestrator"'},
                )
...
def serve(
    ...
    *,
    ...
    """`vector-orchestrator admin serve`. 2 for what stops it starting; 0 when interrupted."""
    try:
    ...
    if store.manifest() is None:
        print(
            f"error: {store_dir} is not a store; run `vector-orchestrator store init` first",
            file=sys.stderr,
        )
        return 2
    try:
