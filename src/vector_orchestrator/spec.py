...
SPEC_ENV = "VECTOR_ORCHESTRATOR_SPEC"
SCHEMA_ENV = "VECTOR_ORCHESTRATOR_STORE_SCHEMA"
#: What each was called before the competitions were named. A deployed host's environment is the
#: one thing not in git, so the old name still answers; the new one wins when both are set.
LEGACY_ENV = {
...
#: refuses `/` itself.
SANDBOX_RESERVED_PATHS = ("/proc", "/sys", "/dev", "/submission", "/run/vector")
...
def _resolve(name: str, env: str) -> Path:
    explicit = os.environ.get(env) or os.environ.get(LEGACY_ENV.get(env, ""))
    if explicit:
...
def validate_spec(doc: dict[str, Any]) -> list[str]:
    ...
    installed one matches its pin is `vector-orchestrator benchmarks check`.
    """
    errors: list[str] = []
