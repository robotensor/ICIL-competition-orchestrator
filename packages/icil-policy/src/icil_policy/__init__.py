"""The ICIL policy protocol: a policy served in its own process, spoken to over named arrays.
...
- `icil_policy.manifest`: `icil.yaml`, which names the policy in a competitor's repository.
- `python -m icil_policy.serve`: serves that policy to one client.
- `icil_policy.client.RemotePolicy`: the client a benchmark drives it with.
- `icil_policy.wire`: the message format between them.
"""
