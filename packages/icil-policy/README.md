...
benchmark that only drives a policy imports `icil_policy.client`, which loads numpy and the
standard library and nothing else.

```bash
pip install -e packages/icil-policy     # from the orchestrator repository
pytest packages/icil-policy             # its tests, on their own
```
...
Unknown or duplicated keys are refused; `icil_policy.manifest.load(path)` lists every problem in
one `ManifestError`. The class implements `icil_policy.Policy`:
...
python -m icil_policy.serve --manifest examples/replay_policy/icil.yaml \
    --address /tmp/policy.sock --authkey-env ICIL_POLICY_AUTHKEY --log-file /tmp/policy.log
```

`--address` is a Unix socket path or `host:port`. The key is read from the environment as hex and
removed from it before competitor code runs. The server accepts one client, builds the policy on
its first `hello` (repository root first on `sys.path` and as the working directory), and exits 0
after `close` or when the client hangs up - even in the middle of a call that never returns. A
...
```python
from icil_policy import PolicyUnavailable
...
`icil_policy.wire`, over `multiprocessing.connection` with an authkey, using `send_bytes` and
